...
initialize_app(cred, {'storageBucket': 'signs-2f063.appspot.com', 'databaseURL':'https://signs-2f063-default-rtdb.firebaseio.com/'})
ref = db.reference("/")
ref.set({'logs': -1})
...
def clean_images():
	file_list = os.listdir('img_dump/')
	for file_name in file_list:
		if '.png' in file_name:
			os.remove('img_dump/' + file_name)
...
def constrastLimit(image):
    ...
    ret_img = cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB)
    return ret_img
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type, LATEST_SIGN):
    sign_change = False
    ...
    if sign is not None:
        ...
        # print(text)
        # print(LATEST_SIGN)
        if text not in ['ERROR', LATEST_SIGN]:
            filename = str(count)+'_'+text+'.png'
            cv2.imwrite('img_dump/' + filename, sign)
            # write image on cloud
            bucket = storage.bucket()
            blob = bucket.blob('img_dump/' + filename)
            blob.upload_from_filename('img_dump/' + filename)
            blob.make_public()
            print("file_url", blob.public_url)

            # write json to shared storage
            ts = datetime.now().timestamp()
            ref.push({'timestamp': ts, 'image': 'gs://signs-2f063.appspot.com/img_dump/' + filename})
    ...
    return coordinate, original_image, sign_type, text, sign_change, text
...
def main(args):
    ...
    while True:
        ...
        coordinate, image, sign_type, text, sign_change, new_sign = localization(frame, args.min_size_components, args.similitary_contour_with_circle, model, count, current_sign, LATEST_SIGN)
        if sign_change:
            LATEST_SIGN = new_sign
